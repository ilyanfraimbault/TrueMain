 ...
 *
 * Forwards both position and the pinned patch (the slice is patch-scoped), keyed
 * on (champion, position, patch) so it dedupes with the rest of the detail page
 * and re-fetches when either filter changes. `server: false` mirrors
 * `useChampion`. `enabled` gates the request until the champion (and its lane)
 * resolves, the same way `useChampionTimelineLeads` does.
 */
export function useChampionScaling(
  ...
  return useLazyAsyncData<ChampionScalingResponse>(
    () => `champion-scaling|${championIdRef.value}|${positionRef.value ?? ''}|${patchRef.value ?? ''}|${eloBracketRef.value ?? ''}`,
    () => {
      ...
      return $fetch<ChampionScalingResponse>(
        `/api/champions/${championIdRef.value}/scaling`,
        { query },
      )
    },
