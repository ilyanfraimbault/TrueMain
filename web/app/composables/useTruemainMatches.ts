 ...
 * <c>UPagination</c> control. Refetches whenever the name tag or page
 * change.
 *
...
export function useTruemainMatches(
  ...
  // Refetch on either nameTag or page change. Empty nameTag short-circuits
  // inside fetchPage so a parent page can bind to a still-empty ref without
  // triggering a 404 round trip on the first tick.
  watch([nameTagRef, pageRef], () => { void fetchPage() }, { immediate: true })
