...
- **Sidebar**: top-10 truemains on the champion; **matchups** (best 5 / worst 5 + specific-opponent picker) with a **Lane / Game** win-rate split (#919 — lane WR counts only *decided* lanes, i.e. a gold gap past `LaneOutcomeAggregation:GoldLeadThreshold` at 15 min, so its sample is smaller than games and shows an em dash rather than 0% when no lane was decided; the lane tooltip also carries the **average gold gap at 15 min** and its own smaller sample, #976). The opponent search **does** carry lane data since #976 — its games stay live while its lane counters are read from the aggregate row for that one opponent — but only on the global route: the player-scoped `?opponent=` keeps them null, since a population-wide aggregate is not one player's lane. **Mains comparison** (`Champion/MainsComparison.vue` — enter a Riot ID, compare WR/KDA/CS·min/gold·min against all mains or one chosen main).
...
**Lane verdict** (#976): once a role opponent is pinned, the confidence strip grows from three cells to five — **lane win rate** and **Gold @15** join it, right beside the win rate they read against, because "we win 54% of these games" and "we end 15 minutes 300 gold up" are the same sentence at two points in the game (it was first shipped as its own strip above the card; that read as a competing verdict and was folded in). The gap carries a badge banding it: Very good / Good / Even / Bad / Hard **lane**, or *matchup* at `JUNGLE`, which has no lane. Both cells come from `GET /champions/{id}/matchups?position=&opponent=` (global slice, every patch) — the matchup aggregate, **not** the composition sample the other three cells count, so their game counts differ on purpose and each carries its own. Keyed on the matchup alone, so a draft edit never refires it. Two shortfall states, never a blank badge: nothing measured yet (`—`), or measured on fewer than `LANE_VERDICT_MIN_GAMES` (10) games — the gap still shows, the verdict does not. Band edges (±150 even, ±300 dominant) live in `web/app/utils/lane-verdict.ts`, deliberately on the read side: they are a product call, and moving them costs a frontend edit instead of a re-fold. Consequence of living in the card: the verdict is absent in the degraded states below, which render the fallback build instead.
