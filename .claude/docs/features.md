...
- **`ProfileActivityHeatmap.vue` — the activity grid** (#927), directly under the LP curve. Ten-column square grid with a four-way mode switch — **game** (last 60), **day** (~30 UTC days), **week** (up to 12 ISO weeks), **patch** — over a single payload (`useTruemainActivity.ts` → `/api/truemains/{tag}/activity`, no mode param, switching is local). Sky fill for a winning period, rosegold for a losing one (`utils/activity-heatmap.ts`); alpha is half decisiveness, half volume. **An empty period has no fill at all** — a hollow outline — so it can never read as a 0% one; its tooltip says "No games". One shared hover readout instead of 60 poppers, plus a coverage line built from the payload's own `source`/`scope` fields: the match-sourced modes state their retained date range and the patch mode states which champion it is scoped to. See the retention-asymmetry entry in `decisions.md`.
