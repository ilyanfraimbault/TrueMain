...
**Lane verdict** (#976, `builder/LaneVerdict.vue`), between the stage and the draft context: a badge — Very good / Good / Even / Bad / Hard **lane** (or *matchup* at `JUNGLE`, which has no lane) — banded from the **average gold gap at 15 minutes**, beside that gap, the lane win rate and the matchup win rate, each with the sample it rests on. The strip owns its fetch (`GET /champions/{id}/matchups?position=&opponent=`, global slice, every patch) rather than riding on the recommendation: it does not refetch when a context slot changes, and it still stands when the build degrades to the fallback below. Two shortfall states, never a blank badge: nothing measured yet, or measured on fewer than `LANE_VERDICT_MIN_GAMES` (10) lanes — the gap still shows, the verdict does not. Band edges (±150 even, ±300 dominant) live in `web/app/utils/lane-verdict.ts`, deliberately on the read side: they are a product call, and moving them costs a frontend edit instead of a re-fold.
