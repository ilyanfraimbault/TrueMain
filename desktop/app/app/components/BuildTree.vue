...
import type { BuildTreeNode } from '~/types/build'

/**
 * Copy of `web/app/components/Champion/BuildPanel/BuildTree.vue` until the
 * shared layer (#1687) lets the desktop import the site's own — the same
 * layout, minus the situational item context the desktop has no slice for.
 * Keep the two in step.
 */
const props = defineProps<{
  tree: BuildTreeNode[]
  firstItemId: number
  /** The core build path: the solid edges trace exactly this. */
  itemPath: number[]
}>()

const { item } = useBuildStatics()
...
<template>
  <div v-if="hasNodes" class="overflow-x-auto">
    <div class="relative mx-auto" :style="{ width: `${layout.width}px`, height: `${layout.height}px` }">
      <svg class="absolute inset-0 overflow-visible" aria-hidden="true" :width="layout.width" :height="layout.height">
        <path
      ...
      <div
        ...
        :title="node.pickRate !== undefined ? `${item(node.itemId)?.name ?? ''} · ${Math.round(node.pickRate * 100)}%` : undefined"
      >
        <GameIcon :source="item(node.itemId)" size="size-9" />
      </div>
    </div>
  </div>
</template>
