...
- **File size is a ratchet, not a style rule.** `.github/scripts/check-file-size.sh` fails CI when a file crosses its limit (500 lines for backend `.cs`, 400 for `.vue`, 300 for frontend `.ts`) or when an already-oversized one grows. The files that were over the limit when the guardrail landed are recorded in `.github/file-size-baseline.txt` and may only shrink — after a split, refresh it with `check-file-size.sh --update` and commit it. Generated code, tests and the dev API mock are out of scope; the exclusion list lives in the script.
